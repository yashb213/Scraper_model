...
const PromotionWrapper = () => {
  return (
    <div className="pb-5">
      <EuiText>
        Unlock the full potential of Google Maps! Get the Pro Version to see
        hidden fields, and enjoy a no questions asked 90-day money-back
        guarantee with lifetime updates.
      </EuiText>
      <EuiButton disabled={true} className="mt-3">
        Get Pro Risk Free
      </EuiButton>
    </div>
  );
    ...
    if (isExecuting) {
      const fetchData = async () => {
        try {
          ...
          const isUpdatedResponse = await Api.isTaskUpdated(
            taskId,
            response.task.updated_at,
